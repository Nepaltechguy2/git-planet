            ...
            temp.open('musicblocks-tmp-', function (err, info) {
                if (!err) {
                    ...
                    fs.close(info.fd, function (err) {
                        exec(`"${config.convert.lilypond}" --pdf --output=${config.convert.outputDir} ${info.path}`, function (err, stdout) {
                            console.log(info.path);
                            temp.cleanupSync();
                            let newFileName = path.basename(info.path).split(".")[0] + fileEnding;
                            let newFileDir = path.join(config.convert.outputDir, newFileName);
                            let b64PDF = fs.readFileSync(newFileDir, 'base64');
                            res.json({
                                success: true,
                                data: {
